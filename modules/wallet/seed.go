...
// nextPrimarySeedAddresses fetches the next n addresses from the primary seed.
func (w *Wallet) nextPrimarySeedAddresses(n uint64, tx *bolt.Tx) ([]types.UnlockConditions, error) {
	...
	// Return only UnlockConditions
	ucs := make([]types.UnlockConditions, 0, len(spendableKeys))
	for i := 0; i < len(spendableKeys); i++ {
		ucs = append(ucs, spendableKeys[i].UnlockConditions)
	}
	return ucs, nil
}
...
func (w *Wallet) nextPrimarySeedAddress(tx *bolt.Tx) (types.UnlockConditions, error) {
	ucs, err := w.nextPrimarySeedAddresses(1, tx)
	return ucs[0], err
}
...
func (w *Wallet) NextAddresses(n uint64) ([]types.UnlockConditions, error) {
	...
	// time.
	w.mu.Lock()
	uc, err := w.nextPrimarySeedAddresses(n, w.dbTx)
